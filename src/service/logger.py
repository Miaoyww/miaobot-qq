...
from src.config.path_config import LOG_PATH
...
class logger:
    ...
    # logger copied from https://github.com/HibiKier/zhenxun_bot
    TEMPLATE_A = "{}"
    TEMPLATE_B = "[<u><c>{}</c></u>]: {}"
    ...
    @classmethod
    def auto(cls, meta: PluginMetadata, event: Event):
        cls.info(f"被触发", user_id=event.get_user_id(), command=meta.name)
